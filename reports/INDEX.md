...
| [v3.0](v3.0_rewrite_and_classify_online_offline.md) | 2026-06-09 | data+model | Nâng task REWRITE → REWRITE_AND_CLASSIFY (phân loại online/offline) | Output mới `{"rewrite_message","domain"}`, tag `<REWRITE_AND_CLASSIFY>`; thêm nguồn ONLINE `generate_online.py` (1514 mẫu) vì data cũ 100% offline; eval thêm `domain_accuracy`+confusion; migrate rẻ data cũ; recipe v5 + notebook synced. Verify local đầy đủ; _pending re-train_ |
